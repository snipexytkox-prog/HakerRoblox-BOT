...
class ZaawansowanyBot(commands.Bot):
    def __init__(self):
        ...
        activity = discord.Activity(type=discord.ActivityType.watching, name="Nowe Zamówienia")
        super().__init__(command_prefix="!", intents=intents, status=discord.Status.online, activity=activity)
...
class ZamowienieModal(ui.Modal, title="FORMULARZ ZAMÓWIENIA USŁUGI"):
    def __init__(self, pakiet_nazwa: str, cena_wyjsciowa: float):
        super().__init__()
        self.pakiet_nazwa = pakiet_nazwa
        self.cena_wyjsciowa = cena_wyjsciowa

    nick_dc = ui.TextInput(label="TWÓJ NICK NA DISCORDZIE: *", placeholder="Np. nazwa_uzytkownika", required=True,
                           max_length=50)
    metoda_platnosci = ui.TextInput(label="WYBRANA METODA PŁATNOŚCI: *", placeholder="BLIK / Przelew", required=True,
                                    max_length=30)
    kod_znizkowy = ui.TextInput(label="KOD RABATOWY (opcjonalnie)", placeholder="Wpisz np. HakerRoblox15", required=False,
                                max_length=20)
    uwagi = ui.TextInput(label="DODATKOWE UWAGI DO ZAMÓWIENIA:", placeholder="Opisz swoje wymagania",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="🟢 POTWIERDZENIE ZAMÓWIENIA",
                              description="Twoje zgłoszenie zamówienia zostało zapisane.", color=discord.Color.green())
        embed.add_field(name="📦 Wybrana usługa:", value=f"• **1x {self.pakiet_nazwa}**", inline=False)

        if rabat_info.startswith("HakerRoblox"):
            embed.add_field(name="💰 Cena:", value=f"~~{self.cena_wyjsciowa:.2f} zł~~ ➡️ **{cena_ostateczna:.2f} zł**",
                            inline=False)
        else:
            embed.add_field(name="💰 Cena:", value=f"**{self.cena_wyjsciowa:.2f} zł**", inline=False)

        embed.add_field(name="👤 Użytkownik:", value=self.nick_dc.value, inline=True)
        ...
        view = ui.View()
        view.add_item(
            ui.Button(label="Opłać zamówienie", url="https://tipply.pl/@hakerroblox", style=discord.ButtonStyle.link, emoji="💳"))
...
class PakietSelect(ui.Select):
    def __init__(self):
        opcje = [
            discord.SelectOption(label="Pakiet START — 19,99 zł",
                                 description="Max 10 kategorii / 30 kanałów + podstawy", emoji="🟢", value="start"),
            discord.SelectOption(label="Pakiet BASIC — 39,99 zł",
                                 description="Max 20 kategorii / 50 kanałów + ekonomia", emoji="🔵", value="basic"),
            discord.SelectOption(label="Pakiet PREMIUM — 69,99 zł",
                                 description="Nielimitowane kategorie i kanały + full opcja", emoji="🟣",
                                 value="premium")
        ]
        super().__init__(placeholder="Wybierz pakiet z menu...", min_values=1, max_values=1, options=opcje,
...
class WyborOfertyView(ui.View):
    def __init__(self): super().__init__(timeout=None); self.add_item(PakietSelect())
...
class MainPanelView(ui.View):
    ...
    async def start_order(self, interaction: discord.Interaction, button: ui.Button):
        embed = discord.Embed(title="🟢 KATALOG OFERT — HAKEROLANDIA",
...
class TicketPanelView(ui.View):
    ...
    async def open_t(self, interaction: discord.Interaction, button: ui.Button):
        ...
        await interaction.followup.send(f"✅ Utworzono dla Ciebie ticket: {ch.mention}", ephemeral=True)
...
class CaptchaModal(ui.Modal):
    def __init__(self, rola_id: int, correct_answer: int, equation_str: str):
        ...
        self.answer_input = ui.TextInput(label=f"Rozwiąż Działanie: {equation_str}", placeholder="Wpisz wynik cyfrą",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("❌ Wynik musi być liczbą całkowitą!", ephemeral=True); return
        if val == self.correct_answer:
            rola = interaction.guild.get_role(self.rola_id)
            if not rola: await interaction.followup.send("❌ Nie znaleziono skonfigurowanej roli na serwerze.",
                                                         ephemeral=True); return
            if rola in interaction.user.roles:
                await interaction.followup.send("⚠️ Posiadasz już tę rolę weryfikacji!", ephemeral=True)
            else:
                await interaction.user.add_roles(rola)
                await interaction.followup.send("✅ Weryfikacja zakończona sukcesem! Otrzymałeś dostęp do serwera.",
                                                ephemeral=True)
        else:
            await interaction.followup.send("❌ Błędny wynik działania! Spróbuj ponownie.", ephemeral=True)


class WeryfikacjaView(ui.View):
    def __init__(self, rola_id: int = None): super().__init__(timeout=None); self.rola_id = rola_id
    ...
    async def verify(self, interaction: discord.Interaction, button: ui.Button):
        if not self.rola_id: await interaction.response.send_message("❌ Brak przypisanej roli weryfikacji.",
                                                                     ephemeral=True); return
...
async def on_message(message):
    ...
    if any(slowo in message.content.lower() for slowo in niedozwolone):
        try:
            ...
            await message.channel.send(f"⚠️ {message.author.mention}, ta treść jest zabroniona na serwerze!",
                                       delete_after=5)
        except:
            pass
...
async def cmd_wyslij_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(description=tresc_panelu, color=discord.Color.from_rgb(114, 137, 218))
    await interaction.channel.send(embed=embed, view=MainPanelView())
    await interaction.followup.send("✅ Pomyślnie wysłano panel sklepu Hakerolandia.", ephemeral=True)


@bot.tree.command(name="ticket-setup", description="[Admin] Wysyła panel ticketów pomocy")
@app_commands.checks.has_permissions(administrator=True)
async def cmd_ticket_setup(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    embed = discord.Embed(title="📩 CENTRUM WSPARCIA — HAKEROLANDIA",
                          description="Kliknij przycisk poniżej, aby otworzyć ticket i skontaktować się z administracją.",
                          color=discord.Color.blue())
    await interaction.channel.send(embed=embed, view=TicketPanelView())
    await interaction.followup.send("✅ Pomyślnie wysłano panel ticketów.", ephemeral=True)


@bot.tree.command(name="weryfikacja-setup", description="[Admin] Wysyła panel weryfikacji użytkowników")
...
@bot.tree.command(name="pomoc", description="[Info] Wyświetla listę wszystkich dostępnych komend")
async def cmd_pomoc(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    embed = discord.Embed(title="📚 POMOC — DOSTĘPNE KOMENDY",
                          description="Wszystkie komendy obsługiwane są jako komendy ukośnika (Slash Commands).",
                          color=discord.Color.blurple())
    embed.add_field(name="🛠️ Administracja i Setup",
...
@bot.tree.command(name="kick", description="[Moderacja] Wyrzuca użytkownika z serwera")
...
@bot.tree.command(name="mute", description="[Moderacja] Wycisza użytkownika na określony czas")
...
@bot.tree.command(name="unmute", description="[Moderacja] Zdejmuje wyciszenie z użytkownika")
@app_commands.checks.has_permissions(moderate_members=True)
...
@bot.tree.command(name="czysc", description="[Moderacja] Masowo usuwa wiadomości z kanału")
...
@bot.tree.command(name="warn", description="[Moderacja] Przyznaje ostrzeżenie użytkownikowi")
@app_commands.checks.has_permissions(manage_messages=True)
async def cmd_warn(interaction: discord.Interaction, użytkownik: discord.Member, powód: str):
    ...
    await interaction.followup.send(
        f"⚠️ Udzielono ostrzeżenia dla {użytkownik.mention}. Łącznie ostrzeżeń: **{user_warnings[użytkownik.id]}**. Powód: {powód}",
        ephemeral=True)


@bot.tree.command(name="ostrzeżenia", description="[Moderacja] Sprawdza liczbę ostrzeżeń użytkownika")
...
@bot.tree.command(name="lock", description="[Admin] Blokuje bieżący kanał przed pisaniem")
...
@bot.tree.command(name="unlock", description="[Admin] Odblokowuje bieżący kanał")
@app_commands.checks.has_permissions(manage_channels=True)
...
@bot.tree.command(name="powoli", description="[Admin] Ustawia tryb powolnego pisania (slowmode)")
@app_commands.checks.has_permissions(manage_channels=True)
...
async def cmd_ping(interaction: discord.Interaction):
    await interaction.response.send_message(f"🏓 Pong! Opóźnienie wynosi: **{round(bot.latency * 1000)} ms**",
                                            ephemeral=True)
...
async def cmd_botinfo(interaction: discord.Interaction):
    await interaction.response.send_message(
        "🤖 Jestem zaawansowanym botem Hakerolandii zarządzającym serwerem, ticketami oraz sklepem w całości po polsku!",
        ephemeral=True)
...
async def cmd_avatar(interaction: discord.Interaction, użytkownik: discord.Member = None):
    ...
    if u.display_avatar: embed.set_image(url=u.display_avatar.url)
    await interaction.followup.send(embed=embed)
...
async def cmd_membercount(interaction: discord.Interaction):
    await interaction.response.send_message(
        f"👥 Aktualna liczba członków na serwerze: **{interaction.guild.member_count}**", ephemeral=True)


@bot.tree.command(name="8ball", description="[4Fun] Magiczna kula odpowie na Twoje pytanie")
@app_commands.describe(pytanie="Zadaj pytanie magicznej kuli")
...
@bot.tree.command(name="rzut-moneta", description="[4Fun] Rzuca monetą (Orzeł czy Reszka)")
...
@bot.tree.command(name="losuj-liczbe", description="[4Fun] Losuje losową liczbę z wybranego zakresu")
...
@bot.tree.command(name="ocena", description="[4Fun] Ocenia podaną rzecz w skali od 0 do 10")
...
@bot.tree.command(name="żart", description="[4Fun] Opowiada losowy żart po polsku")
async def cmd_joke(interaction: discord.Interaction):
    zarty = [
        "– Dlaczego programiści nie lubią natury? – Bo ma za dużo robaków (bugów)!",
        "– Ile języków programowania zna dobry informatyk? – Jeden, ale potrafi w nim kląć na sto sposobów.",
        "– Czym się różni hydraulik od programisty? – Hydraulik nie pyta, czy próbowałeś zakręcić wodę i odkręcić ponownie."
    ]
    await interaction.response.send_message(random.choice(zarty))


@bot.tree.command(name="przytul", description="[4Fun] Przytula wybranego użytkownika")
async def cmd_hug(interaction: discord.Interaction, użytkownik: discord.Member):
    await interaction.response.send_message(
        f"🤗 {interaction.user.mention} mocno przytula użytkownika {użytkownik.mention}!")


@bot.tree.command(name="ciastko", description="[4Fun] Częstuje kogoś wirtualnym ciastkiem")
async def cmd_cookie(interaction: discord.Interaction, użytkownik: discord.Member):
    await interaction.response.send_message(
        f"🍪 {interaction.user.mention} częstuje wirtualnym ciastkiem użytkownika {użytkownik.mention}!")


@bot.tree.command(name="portfel", description="[Ekonomia] Sprawdza stan konta monet")
...
@bot.tree.command(name="praca", description="[Ekonomia] Podejmij pracę i zarób losową ilość monet")
async def cmd_work(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"💼 Wykonałeś pracę i zarobiłeś **{zarobek} monet**!")


@bot.tree.command(name="codzienna-nagroda", description="[Ekonomia] Odbierz darmowy bonus monet")
async def cmd_daily(interaction: discord.Interaction):
    bonus = 500
    user_balances[interaction.user.id] = user_balances.get(interaction.user.id, 0) + bonus
    await interaction.response.send_message(f"🎁 Odebrałeś codzienne wsparcie w wysokości **{bonus} monet**!")


@bot.tree.command(name="przelej", description="[Ekonomia] Przelej monety innym użytkownikom")
async def cmd_pay(interaction: discord.Interaction, użytkownik: discord.Member, kwota: int):
    if kwota <= 0:
        await interaction.response.send_message("❌ Kwota przelewu musi być większa od zera.", ephemeral=True)
        return
    uid = interaction.user.id
    if user_balances.get(uid, 0) < kwota:
        await interaction.response.send_message("❌ Nie posiadasz tyle monet w portfelu!", ephemeral=True)
        return
    user_balances[uid] -= kwota
    user_balances[użytkownik.id] = user_balances.get(użytkownik.id, 0) + kwota
    await interaction.response.send_message(
        f"✅ Pomyślnie przelałeś **{kwota} monet** dla użytkownika {użytkownik.mention}.")
