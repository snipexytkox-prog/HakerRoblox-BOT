...
class ZamowienieModal(ui.Modal, title="FORMULARZ ZAMÓWIENIA USŁUGI"):
    ...
    metoda_platnosci = ui.TextInput(
        ...
        placeholder="BLIK / Przelew",
        required=True,
        max_length=30,
    )
    kod_znizkowy = ui.TextInput(
        label="KOD RABATOWY (opcjonalnie)",
        placeholder="Wpisz np. HakerRoblox15",
        required=False,
        max_length=20,
    )
    uwagi = ui.TextInput(
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="💳 Płatność:", value=self.metoda_platnosci.value, inline=True
        )
        embed.add_field(
            name="🎟️ Kod rabatowy:", value=rabat_info, inline=False
        )
...
# =========================================================================
# --- 5. KOMENDY (Admin/Mod ograniczone do WŁAŚCICIELA) ---
# =========================================================================
...
async def cmd_pomoc(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value=(
            "`/ban`, `/kick`, `/mute`, `/unmute`, `/czysc`, `/warn`,"
            " `/ostrzeżenia`"
        ),
        inline=False,
    )
    embed.add_field(
        name="📊 Informacje",
        value=(
            "`/ping`, `/serwer-info`, `/użytkownik-info`, `/bot-info`,"
            " `/avatar`, `/liczba-osób`, `/regulamin`"
        ),
        inline=False,
    )
    embed.add_field(
        name="🎮 Zabawa (4Fun)",
        value=(
            "`/8ball`, `/rzut-moneta`, `/losuj-liczbe`, `/ocena`, `/żart`,"
            " `/przytul`, `/ciastko`"
        ),
        inline=False,
    )
...
async def cmd_regulamin(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="🤝 1. Szanuj innych",
        value=(
            "Nie wyzywaj, nie obrażaj i nie prowokuj. Zakaz mowy nienawiści,"
            " rasizmu i dyskryminacji."
        ),
        inline=False,
    )
    embed.add_field(
        name="🛡️ 2. Szanuj administrację",
        value=(
            "Wykonuj polecenia administracji. Jeśli nie zgadzasz się z"
            " decyzją, zgłoś problem przez ticket."
        ),
        inline=False,
    )
    embed.add_field(
        name="💬 3. Nie spamuj",
        value=(
            "Zakaz spamu, floodu, bezsensownego pingowania oraz pisania nie na"
            " temat. Za spam grozi 1 godzina przerwy (mute)."
        ),
        inline=False,
    )
    embed.add_field(
        name="📢 4. Zakaz reklam i własnych serwerów",
        value=(
            "Nie reklamuj innych serwerów, kanałów, stron ani usług bez zgody"
            " administracji. Reklamowanie własnego serwera Discord jest"
            " zabronione — wysyłanie zaproszeń do własnych serwerów = ban. Zakaz"
            " podejrzanych linków, cheatów i exploitów."
        ),
        inline=False,
    )
    embed.add_field(
        name="🔞 5. Zakaz treści 18+",
        value=(
            "Nie wysyłaj treści NSFW, seksualnych, brutalnych ani innych"
            " nieodpowiednich materiałów."
        ),
        inline=False,
    )
    embed.add_field(
        name="🔐 6. Chroń prywatność",
        value=(
            "Nie udostępniaj danych swoich ani innych osób. Zakaz publikowania"
            " prywatnych zdjęć i podszywania się pod innych."
        ),
        inline=False,
    )
    embed.add_field(
        name="🎮 7. Graj uczciwie",
        value=(
            "Zakaz cheatów, exploitów i wykorzystywania błędów w celu uzyskania"
            " przewagi."
        ),
        inline=False,
    )
    embed.add_field(
        name="🧵 8. Zakaz tworzenia wątków",
        value=(
            "Tworzenie wątków na serwerze jest zabronione. Za utworzenie"
            " wątku grozi kick."
        ),
        inline=False,
    )
    embed.add_field(
        name="🎫 9. Zgłoszenia",
        value=(
            "Problemy i skargi zgłaszaj przez ticket. Nie oznaczaj"
            " administracji bez ważnego powodu."
        ),
        inline=False,
    )
    embed.add_field(
        name="⚠️ 10. Kary",
        value=(
            "Ostrzeżenie → Mute → Kick → Ban. Za poważne przewinienia kara"
            " może zostać nadana od razu. Omijanie bana = ban permanentny."
        ),
        inline=False,
    )
    embed.add_field(
        name="✅ 11. Akceptacja",
        value=(
            "Dołączając na serwer, akceptujesz regulamin i zobowiązujesz się go"
            " przestrzegać."
        ),
        inline=False,
    )
...
@bot.tree.command(
    name="ban", description="[Właściciel] Banuje wybranego użytkownika"
)
@is_owner()
async def cmd_ban(
    interaction: discord.Interaction,
    użytkownik: discord.Member,
    powód: str = "Brak podanego powodu",
):
    await interaction.response.defer(ephemeral=True)
    await użytkownik.ban(reason=powód)
    await interaction.followup.send(
        f"🔨 Zbanowano użytkownika {użytkownik.mention}. Powód: **{powód}**",
        ephemeral=True,
    )


@bot.tree.command(
    name="kick", description="[Właściciel] Wyrzuca użytkownika z serwera"
)
@is_owner()
async def cmd_kick(
    interaction: discord.Interaction,
    ...
    await interaction.followup.send(
        f"👢 Wyrzucono użytkownika {użytkownik.mention}. Powód: **{powód}**",
        ephemeral=True,
    )


@bot.tree.command(
    name="mute", description="[Właściciel] Wycisza użytkownika na określony czas"
)
@is_owner()
async def cmd_mute(
    interaction: discord.Interaction,
    ...
    await interaction.followup.send(
        f"🔇 Wyciszono użytkownika {użytkownik.mention} na {minuty} minut.",
        ephemeral=True,
    )


@bot.tree.command(
    name="unmute", description="[Właściciel] Zdejmuje wyciszenie z użytkownika"
)
@is_owner()
async def cmd_unmute(interaction: discord.Interaction, użytkownik: discord.Member):
    ...
    await interaction.followup.send(
        f"🔊 Odciszono użytkownika {użytkownik.mention}.", ephemeral=True
    )


@bot.tree.command(
    name="czysc", description="[Właściciel] Masowo usuwa wiadomości z kanału"
)
@is_owner()
async def cmd_czysc(interaction: discord.Interaction, ilosc: int):
    if ilosc < 1 or ilosc > 100:
        await interaction.response.send_message(
            "❌ Możesz usunąć od 1 do 100 wiadomości naraz.", ephemeral=True
        )
        return
    await interaction.response.defer(ephemeral=True)
    usuniete = await interaction.channel.purge(limit=ilosc)
    await interaction.followup.send(
        f"✅ Pomyślnie usunięto **{len(usuniete)}** wiadomości.", ephemeral=True
    )


@bot.tree.command(
    name="warn", description="[Właściciel] Przyznaje ostrzeżenie użytkownikowi"
)
@is_owner()
async def cmd_warn(
    interaction: discord.Interaction, użytkownik: discord.Member, powód: str
):
    await interaction.response.defer(ephemeral=True)
    user_warnings[użytkownik.id] = user_warnings.get(użytkownik.id, 0) + 1
    await interaction.followup.send(
        f"⚠️ Udzielono ostrzeżenia dla {użytkownik.mention}. Łącznie ostrzeżeń:"
        f" **{user_warnings[użytkownik.id]}**. Powód: {powód}",
        ephemeral=True,
    )
...
@bot.tree.command(
    name="ostrzeżenia",
    description="[Info] Sprawdza liczbę ostrzeżeń użytkownika",
)
async def cmd_ostrzeżenia(
    interaction: discord.Interaction, użytkownik: discord.Member = None
):
    await interaction.response.defer(ephemeral=True)
    cel = użytkownik or interaction.user
    liczba = user_warnings.get(cel.id, 0)
    await interaction.followup.send(
        f"📌 Użytkownik {cel.mention} posiada **{liczba}** ostrzeżeń.",
        ephemeral=True,
    )


@bot.tree.command(
    name="lock", description="[Właściciel] Blokuje bieżący kanał przed pisaniem"
)
@is_owner()
async def cmd_lock(interaction: discord.Interaction):
    ...
    await interaction.followup.send(
        "🔒 Ten kanał został pomyślnie zablokowany.", ephemeral=True
    )


@bot.tree.command(
    name="unlock", description="[Właściciel] Odblokowuje bieżący kanał"
)
@is_owner()
async def cmd_unlock(interaction: discord.Interaction):
    ...
    await interaction.followup.send(
        "🔓 Ten kanał został odblokowany.", ephemeral=True
    )


@bot.tree.command(
    name="powoli",
    description="[Właściciel] Ustawia tryb powolnego pisania (slowmode)",
)
@is_owner()
async def cmd_powoli(interaction: discord.Interaction, sekundy: int):
    ...
    await interaction.followup.send(
        f"⏱️ Ustawiono tryb powolnego pisania na **{sekundy} sekund**.",
        ephemeral=True,
    )


@bot.tree.command(
    name="ping", description="[Info] Sprawdza opóźnienie (ping) bota"
)
async def cmd_ping(interaction: discord.Interaction):
    await interaction.response.send_message(
        f"🏓 Pong! Opóźnienie wynosi: **{round(bot.latency * 1000)} ms**",
        ephemeral=True,
    )


@bot.tree.command(
    name="serwer-info", description="[Info] Wyświetla informacje o obecnym serwerze"
)
async def cmd_serwerinfo(interaction: discord.Interaction):
    await interaction.response.defer()
    g = interaction.guild
    embed = discord.Embed(
        title=f"📊 Informacje o serwerze: {g.name}", color=discord.Color.green()
    )
    embed.add_field(
        name="Właściciel",
        value=g.owner.mention if g.owner else "Brak danych",
        inline=True,
    )
...
@bot.tree.command(
    name="użytkownik-info",
    description="[Info] Wyświetla informacje o wybranym użytkowniku",
)
async def cmd_userinfo(
    ...
    embed = discord.Embed(
        title=f"👤 Informacje o: {u.name}", color=discord.Color.blue()
    )
    embed.add_field(name="ID Użytkownika", value=u.id, inline=True)
    embed.add_field(
        name="Konto utworzone", value=u.created_at.strftime("%d.%m.%Y"), inline=True
    )
    await interaction.followup.send(embed=embed)
...
@bot.tree.command(
    name="avatar", description="[Info] Pokazuje awatar wybranego użytkownika"
)
async def cmd_avatar(
    ...
    u = użytkownik or interaction.user
    embed = discord.Embed(
        title=f"🖼️ Awatar użytkownika {u.name}", color=discord.Color.purple()
    )
    if u.display_avatar:
...
@bot.tree.command(
    name="liczba-osób",
    description="[Info] Pokazuje szybki licznik członków na serwerze",
)
async def cmd_membercount(interaction: discord.Interaction):
    await interaction.response.send_message(
        f"👥 Aktualna liczba członków na serwerze:"
...
@bot.tree.command(
    name="8ball", description="[4Fun] Magiczna kula odpowie na Twoje pytanie"
)
...
async def cmd_8ball(interaction: discord.Interaction, pytanie: str):
    ...
    embed.add_field(
        name="Odpowiedź:", value=random.choice(odpowiedzi), inline=False
    )
    await interaction.response.send_message(embed=embed)


@bot.tree.command(
    name="rzut-moneta", description="[4Fun] Rzuca monetą (Orzeł czy Reszka)"
)
async def cmd_coinflip(interaction: discord.Interaction):
    await interaction.response.send_message(
        f"🪙 Wylosowano: **{random.choice(['Orzeł', 'Reszka'])}**!"
    )


@bot.tree.command(
    name="losuj-liczbe",
    description="[4Fun] Losuje losową liczbę z wybranego zakresu",
)
async def cmd_roll(interaction: discord.Interaction, maksimum: int = 100):
    await interaction.response.send_message(
        f"🎲 Wylosowana liczba (1-{maksimum}):"
        f" **{random.randint(1, maksimum)}**"
    )


@bot.tree.command(
    name="ocena", description="[4Fun] Ocenia podaną rzecz w skali od 0 do 10"
)
async def cmd_rate(interaction: discord.Interaction, tekst: str):
    await interaction.response.send_message(
        f"⭐ Ocena dla **{tekst}**: **{random.randint(0, 10)}/10**!"
    )


@bot.tree.command(
    name="żart", description="[4Fun] Opowiada losowy żart po polsku"
)
async def cmd_joke(interaction: discord.Interaction):
    zarty = [
        (
            "– Dlaczego programiści nie lubią natury? – Bo ma za dużo robaków"
            " (bugów)!"
        ),
        (
            "– Ile języków programowania zna dobry informatyk? – Jeden, ale"
            " potrafi w nim kląć na sto sposobów."
        ),
        (
            "– Czym się różni hydraulik od programisty? – Hydraulik nie pyta,"
            " czy próbowałeś zakręcić wodę i odkręcić ponownie."
        ),
    ]
...
@bot.tree.command(
    name="przytul", description="[4Fun] Przytula wybranego użytkownika"
)
async def cmd_hug(interaction: discord.Interaction, użytkownik: discord.Member):
    await interaction.response.send_message(
        f"🤗 {interaction.user.mention} mocno przytula użytkownika"
        f" {użytkownik.mention}!"
    )


@bot.tree.command(
    name="ciastko", description="[4Fun] Częstuje kogoś wirtualnym ciastkiem"
)
async def cmd_cookie(
    interaction: discord.Interaction, użytkownik: discord.Member
):
    await interaction.response.send_message(
        f"🍪 {interaction.user.mention} częstuje wirtualnym ciastkiem"
        f" użytkownika {użytkownik.mention}!"
    )


@bot.tree.command(
    name="portfel", description="[Ekonomia] Sprawdza stan konta monet"
)
async def cmd_balance(interaction: discord.Interaction):
    bal = user_balances.get(interaction.user.id, 0)
    await interaction.response.send_message(
        f"💰 W Twoim portfelu znajduje się: **{bal} monet**.", ephemeral=True
    )


@bot.tree.command(
    name="praca", description="[Ekonomia] Podejmij pracę i zarób losową ilość monet"
)
async def cmd_work(interaction: discord.Interaction):
    zarobek = random.randint(30, 150)
    user_balances[interaction.user.id] = (
        user_balances.get(interaction.user.id, 0) + zarobek
    )
    await interaction.response.send_message(
        f"💼 Wykonałeś pracę i zarobiłeś **{zarobek} monet**!"
    )


@bot.tree.command(
    name="codzienna-nagroda", description="[Ekonomia] Odbierz darmowy bonus monet"
)
async def cmd_daily(interaction: discord.Interaction):
    bonus = 500
    user_balances[interaction.user.id] = (
        user_balances.get(interaction.user.id, 0) + bonus
    )
    await interaction.response.send_message(
        f"🎁 Odebrałeś codzienne wsparcie w wysokości **{bonus} monet**!"
    )


@bot.tree.command(
    name="przelej", description="[Ekonomia] Przelej monety innym użytkownikom"
)
async def cmd_pay(
    interaction: discord.Interaction, użytkownik: discord.Member, kwota: int
):
    if kwota <= 0:
        await interaction.response.send_message(
            "❌ Kwota przelewu musi być większa od zera.", ephemeral=True
        )
        return
    uid = interaction.user.id
    if user_balances.get(uid, 0) < kwota:
        await interaction.response.send_message(
            "❌ Nie posiadasz tyle monet w portfelu!", ephemeral=True
        )
        return
    user_balances[uid] -= kwota
    user_balances[użytkownik.id] = (
        user_balances.get(użytkownik.id, 0) + kwota
    )
    await interaction.response.send_message(
        f"✅ Pomyślnie przelałeś **{kwota} monet** dla użytkownika"
        f" {użytkownik.mention}."
    )
