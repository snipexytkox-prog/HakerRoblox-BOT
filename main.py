...
class ZaawansowanyBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        if GUILD_ID:
            ...
            print("✅ Pomyślnie zsynchronizowano komendy dla serwera testowego!")
        else:
...
# --- 1. SYSTEM SKLEPU I ZAMÓWIEŃ Z MENU PŁATNOŚCI W MODALU ---
class ZamowienieModal(ui.Modal):
    ...
    # Menu wyboru płatności wewnątrz modalu
    platnosc_select = ui.Select(
        placeholder="Wybierz metodę płatności...",
        min_values=1,
        max_values=1,
        options=[
            discord.SelectOption(label="BLIK", description="Szybka płatność BLIK", emoji="💳"),
            discord.SelectOption(label="REVOLUT", description="Płatność przez Revolut", emoji="💸"),
        ]
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Pobieranie wybranej opcji z menu select w modalu
        wybrana_platnosc = self.platnosc_select.values[0] if self.platnosc_select.values else "Nie wybrano"
        ...
        if rabat_info.startswith("HakerRoblox"):
            embed.add_field(name="💰 Cena:", value=f"~~{self.cena_wyjsciowa:.2f} zł~~ ➡️ **{cena_ostateczna:.2f} zł**", inline=False)
        else:
...
class TicketPanelView(ui.View):
    ...
    async def open_t(self, interaction: discord.Interaction, button: ui.Button):
        ...
        embed = discord.Embed(title="CENTRUM POMOCY", description=f"Witaj {interaction.user.mention}!", color=discord.Color.blue())
...
class CaptchaModal(ui.Modal):
    def __init__(self, rola_id: int, correct_answer: int, equation_str: str):
        ...
        self.answer_input = ui.TextInput(label=f"Rozwiąż Działanie: {equation_str}", placeholder="Wynik", required=True, max_length=5)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if val == self.correct_answer:
            ...
            if rola:
                ...
                await interaction.followup.send("✅ Weryfikacja powiodła się!", ephemeral=True)
        else:
...
async def cmd_wyslij_panel(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    embed = discord.Embed(title="🖥️ ZAMÓW SWÓJ SERWER", description="Kliknij przycisk poniżej, aby złożyć zamówienie.", color=discord.Color.blurple())
    await interaction.channel.send(embed=embed, view=MainPanelView())
    await interaction.followup.send("✅ Wysłano panel.", ephemeral=True)
